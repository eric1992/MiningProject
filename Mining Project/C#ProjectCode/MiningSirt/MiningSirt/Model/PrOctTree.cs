...
namespace MiningSirt.Model
{
    class PrOctTree<T> where T : Compare3D<T>
    {
	/**
	 ...
	 *
	 */
	class prOctNodeInternal : prOctNode{
		public prOctNode UNW, UNE, USE, USW, DNW, DNE, DSE, DSW;
	}
	...
	private prOctNode insertHelper(T elem, prOctNode root, double xMax,
		...
		else if(root is prOctNodeInternal){
	...
	private T findHelper(T elem, prOctNode root, double xMax, double xMin, double yMax, double yMin, double zMax, double zMin){
		if(root == null || !elem.inBox(xMin, xMax, yMin, yMax, zMin, zMax))
			...
			for(T x: handle.Elements)
				//if any of the elements in the List equal the search item, return this node.
				if(x.equals(elem))
					return x;
			return null;
		}
	 ...
	 * @return
	 */
	public List<T> findOnLine(Point a, Point b){
	...
	private List<T> findOnLineHelper(prOctNode root, Point a, Point b, double xMin,
